...
export class GameService {
  ...
  /**
   * Costo en monedas de cada comodín. Centralizado acá para poder ajustarlo
   * en un solo lugar (a futuro podría venir de configuración/entorno).
   */
  private static readonly LifelineCosts: Record<string, number> = {
    fifty_fifty: 30,
    audience: 25,
    add_time: 15,
  };

  private static readonly AddTimeSeconds = 10;

  /**
   * Comodines. Cobran monedas al usuario (en el servidor, atómicamente) y
   * fallan si el saldo no alcanza. Los que requieren conocer la respuesta
   * correcta (50/50, público) también se resuelven acá para no filtrarla.
   *  - fifty_fifty: devuelve hasta 2 respuestas incorrectas para ocultar.
   *  - audience: devuelve una distribución de "votos" sesgada hacia la correcta.
   *  - add_time: solo cobra y confirma los segundos extra (la app los aplica).
   */
  async lifeline(userId: string, dto: LifelineDto): Promise<LifelineResponse> {
    const cost = GameService.LifelineCosts[dto.type] ?? 0;

    return this.uow.execute(async (repos) => {
      const session = await repos.gameSessions.findById(dto.sessionId);
      if (!session) throw new NotFoundException('La partida no existe.');
      if (session.userId !== userId) throw new ForbiddenException('La partida no pertenece a este usuario.');
      if (session.status !== GameSessionStatus.InProgress) {
        throw new BadRequestException('La partida ya finalizó.');
      ...
      // Cobro del comodín. Si no alcanza, spendCoins lanza DomainException → 400.
      user.spendCoins(cost);
      await repos.users.update(user);

      if (dto.type === 'add_time') {
        return { addedSeconds: GameService.AddTimeSeconds, cost, coins: user.coins };
      }

      const question = await repos.questions.findWithAnswers(dto.questionId);
      if (!question) throw new NotFoundException('La pregunta no existe.');
      ...
      if (dto.type === 'fifty_fifty') {
        ...
        return { removeAnswerIds, cost, coins: user.coins };
      }

      const audience = this.buildAudience(answers.map((a) => ({ id: a.id, isCorrect: a.isCorrect })));
      return { audience, cost, coins: user.coins };
    });
  }
